...
volatile int* to_be_done_by = NULL;
volatile int* counter_of_not_finished = NULL;

int critical_mode = 0;

volatile task input[] = { //{.T, .t0, .C, .x} x=1 stvarni ulaz, x=0 popuna (LAB2)
...
int main()
{
	...
	to_be_done_by = (int*)malloc(N*sizeof(int));			// Array for keeping track when a current iteration of task i has to finish
	counter_of_not_finished = (int*)malloc(N*sizeof(int));		// Array for keeping count of consecutive not finished iterations of task i
...
void* simulator_function(void* arg)
{
	...
	while(1)
	{
		...
		to_be_done_by[i] = t + my_task.T;
		state[i] = 1;						// Input
		t_change_start = clock_main;
		PRINT("%d\t\t\t\tInput %d: New input to be done by: %d\n", t_change_start, i, to_be_done_by[i]);
...
{	
	...
	while(1)
	{
		...
		for(int i = 0; i < N; i++)
		{
			if((critical_mode == 0) || ((critical_mode == 1) && (counter_of_not_finished[i] > 0)))
			{
				if(critical_mode == 0 && counter_of_not_finished[i] >= 3)
				{
					printf("\n\nCRITICAL MODE %d\n\n", i);
					critical_mode = 1;
					printf("\nList of tasks not done in last turn:\n");
					for(int j = 0; j < N; j++)
					{
						if(counter_of_not_finished[j] > 0)
							printf("%d ", j);
					}
					printf("\n");
				}
				if(state[i] == 1)
				{	
					if(input[i].C > (to_be_done_by[i] - clock_main))
					{
						printf("%d\t%d This input cannot be processed in time(%d, %d, %d), aborting!\n", clock_main, i, input[i].C, to_be_done_by[i], clock_main);
					}
					else
					{
						struct timespec request = {0, input[i].C * 1000000};
						PRINT__(2, "%d\tWorking on input %d, to_be_done_by: %d, C: %d\tcounter: %d\n", clock_main, i, to_be_done_by[i], input[i].C, counter_of_not_finished[i]);
		...
		if(critical_mode == 1)
		{
			for(int i = 0; i < N; i++)
			{
				if(counter_of_not_finished[i] > 1)
					flag = 1;
			}
			if(flag == 0){
				printf("\n\nNORMAL MODE\n\n");
